...
def protocol_client(data):
    ...
    client_logger.debug(f'Подготовлен запрос "{dict_client[ACTION]}" от "{dict_client[USER][ACCOUNT_NAME]}" для отправки серверу')
    return dict_client
...
def analysis_answer(dict_data, msg_service=None):
    if dict_data['response'] == '200':
        ...
        client_logger.debug(f'Получен ответ "{dict_data[RESPONSE]} {dict_data[ALERT]}" от сервера (правильный запрос/JSON-объект).')
    elif dict_data[RESPONSE] == '400':
        print(f'{dict_data[RESPONSE]} {dict_data[ALERT]} {dict_data[TIME]} Неправильный запрос!')
        client_logger.debug(f'Получен ответ "{dict_data[RESPONSE]} {dict_data[ALERT]}" от сервера (неправильный запрос/JSON-объект).')
